...
function () {
  _createClass(Item, null, [{
    ...
    value: function load(data) {
      ...
      var relationshipItems = (data.includes.Entry || []).map(function (item) {
        var Klass = _this.getClass(item.sys.contentType.sys.id);

        return new Klass(item);
      });
      var assets = (data.includes.Asset || []).map(function (item) {
        return new Asset(item);
      });
      var primaries = data.items.map(function (item) {
  ...
  _createClass(Item, [{
    ...
    value: function setSingularRelationship(name, id, items) {
      ...
      if (lodash_4(match)) {
        throw new Error("No matching record found for ".concat(id, " on ").concat(this));
      }
    ...
    value: function setPluralRelationship(name, ids, items) {
      ...
      var matches = ids.map(function (id) {
        ...
        if (lodash_4(match)) {
          throw new Error("No matching record found for ".concat(id, " on ").concat(_this2));
        }
    ...
    value: function toJSON() {
      ...
      var fields = this.constructor.fields.reduce(function (accumulator, field) {
        accumulator[field] = _this4.get(field);
        return accumulator;
      }, {});
