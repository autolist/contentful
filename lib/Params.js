...
export default class {
  ...
  get skip() {
    ...
    if (!isUndefined(this.params.page) && isUndefined(this.limit)) {
      throw new Error(
        `You must specify a limit to use the page param, limit param is currently ${this.params.limit}`
      );
    }
    ...
    if (!isFinite(page) || !isFinite(this.limit)) {
      throw new Error(
        `Both page and limit params must be numbers. Currently they are ${this.params.page} and ${this.params.limit}
        `
      );
    }
