...
const fieldIsLink = field =>
  isArray(field) || get(field, 'sys.type') === 'Link' || get(field, 'sys.id');
...
export default class Item {
  ...
  static getClass(contentType) {
    ...
    logger.log({
      level: 'warn',
      message: `No class found for content type ${contentType}, falling back to default ${
        Item.name
      }`
    });
  ...
  relationshipToJSON(fieldName) {
    ...
    if (isEmpty(relationship)) {
      logger.log({
        level: 'warn',
        message: `Relationship ${fieldName} is empty`
      });
      return undefined;
    }
