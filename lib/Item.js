...
export default class Item {
  ...
  static getClass(contentType) {
    ...
    logger(
      'warn',
      `No class found for content type ${contentType}, falling back to default ${Item.name}`
    );
  ...
  static load(data) {
    const includedItems = get(data, 'includes.Entry', []).map(item => {
    ...
    const primaries = data.items.map(item => new this(item));

    [...primaries, ...includedItems].forEach(item =>
      item.mapRelationships([...primaries, ...includedItems, ...assets])
    );
